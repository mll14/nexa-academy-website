  ...
  return (
    <div className="overflow-auto">
      ...
      <div className="relative" style={{ height: SLOT_HEIGHT * TOTAL_SLOTS }}>
        {slots.map((label, i) => (
          <div key={label} style={{ top: i * SLOT_HEIGHT, height: SLOT_HEIGHT }}
            className="absolute inset-x-0 border-t border-border/40 flex items-start">
            <span className="text-[10px] text-muted-foreground w-10 shrink-0 mt-0.5 ml-1 select-none">{label}</span>
          </div>
        ))}
        {timed.map((ev) => {
          const { top, height } = positionStyle(ev)
          return (
            <div key={ev.id} style={{ top, height, left: 44, right: 4, position: 'absolute' }} className="pr-0.5">
